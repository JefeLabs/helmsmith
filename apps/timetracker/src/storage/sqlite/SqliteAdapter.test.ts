...
import { figmaStorageContract } from '../figma-contract.js';
...
// SQLite is currently the only backend implementing the FigmaStorage port.
figmaStorageContract('SqliteAdapter (:memory:)', async () => {
  const adapter = new SqliteAdapter({ backend: 'sqlite', path: ':memory:' });
  await adapter.init();
  return adapter;
});
