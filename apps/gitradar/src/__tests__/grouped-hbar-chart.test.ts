...
describe('renderGroupedHBarChart', () => {
  ...
  it('renders multiple groups with multiple bars', () => {
    ...
    const teamBLines = lines.filter((l) => stripAnsi(l).includes('Team B'));
  ...
  it('renders dashed separator after specified bar indices', () => {
    ...
    const separatorIndex = plainLines.findIndex((l) => l.includes('\u2500 \u2500'));
    const frontSquadIndex = plainLines.findIndex((l) => l.includes('FrontSquad'));
  ...
  it('displays formatted values right of bars when showValues is true', () => {
    const groups: HBarGroup[] = [
      {
        groupLabel: 'W12',
        bars: [makeBar('Team A', { app: 15100 }), makeBar('Team B', { app: 4400 })],
      },
    ];
    const result = renderGroupedHBarChart({
      groups,
  ...
  it('ensures non-zero segments get minimum 1 char', () => {
    ...
    const groups: HBarGroup[] = [
      {
        groupLabel: 'W12',
        bars: [makeBar('Team A', { app: 10000, storybook: 1 })],
      },
    ];
    const result = renderGroupedHBarChart({
      groups,
  ...
  it('scales bars proportionally to globalMax', () => {
    const groups: HBarGroup[] = [
      {
        groupLabel: 'W12',
        bars: [makeBar('Big', { app: 1000 }), makeBar('Small', { app: 500 })],
      },
    ];
    const result = renderGroupedHBarChart({
      groups,
    ...
    const bigBarChars = bigLine.split('').filter((c) => c === '\u2588').length;
    const smallBarChars = smallLine.split('').filter((c) => c === '\u2588').length;
  ...
  it('aligns value columns across bars of different widths', () => {
    const groups: HBarGroup[] = [
      {
        groupLabel: 'W12',
        bars: [makeBar('Big Team', { app: 8000 }), makeBar('Small Team', { app: 2000 })],
      },
    ];
