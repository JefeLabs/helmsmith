...
describe('Functional: Full CLI Pipeline (Engine + SQLite)', () => {
  ...
  it('Step 7: assign known authors to SkoolScout org via SQLite', async () => {
    ...
    const { loadAuthorRegistrySQL, saveAuthorRegistrySQL, reattributeRecordsSQL, queryRecords } =
      await import('../store/sqlite-store.js');

    const _config = await loadConfigFromTemp();
  ...
  it('Step 8: queryRollup by member has non-zero metrics', async () => {
    ...
    console.log(
      `  queryRollup: ${rolled.size} members, ${teamNames.length} teams: [${teamNames.join(', ')}]`,
    );
  });
  ...
  it('Step 9: queryRollup by org shows SkoolScout with data', async () => {
    ...
    const ins =
      skoolscout!.filetype.app.insertions +
      skoolscout!.filetype.test.insertions +
      skoolscout!.filetype.config.insertions +
      skoolscout!.filetype.storybook.insertions;
    expect(ins).toBeGreaterThan(0);

    console.log(`  SkoolScout (SQL rollup): ${skoolscout!.commits} commits, ${ins} insertions`);
  });
  ...
  it('Step 10: queryRollup by repo shows data for scanned repos', async () => {
    ...
    const ins =
      skoolscoutCom.filetype.app.insertions +
      skoolscoutCom.filetype.test.insertions +
      skoolscoutCom.filetype.config.insertions +
      skoolscoutCom.filetype.storybook.insertions;
    expect(ins).toBeGreaterThan(0);
  ...
  it('Step 14: re-attribution is idempotent', async () => {
    ...
    console.log(
      `  Re-attribution idempotent: ${assignedAfter} assigned, ${unassignedAfter} unassigned`,
    );
  });
  ...
  it('Step 15: upsertRecords merges duplicate keys additively in SQLite', async () => {
    const { queryRecords, upsertRecords, getStoreStatsSQLFull } = await import(
    ...
    for (const orig of originals) {
      const found = reloaded.find((r) => `${r.member}::${r.week}::${r.repo}` === orig.key);
    ...
    console.log(
      `  upsertRecords: ${subset.length} records merged, count unchanged at ${after.recordCount}`,
    );
  });
