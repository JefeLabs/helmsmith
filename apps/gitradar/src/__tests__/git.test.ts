...
function createMockChildProcess(
  ...
  process.nextTick(() => {
    if (outputOrError instanceof Error) {
      ...
      const data =
        outputOrError && !outputOrError.endsWith('\n') ? `${outputOrError}\n` : outputOrError;
...
describe('parseGitLogOutput', () => {
  ...
  it('handles binary files (- for insertions/deletions)', () => {
    const output = ['ccc333|alice@acme.com|Alice|2026-02-22T10:00:00Z', '-\t-\timage.png'].join(
      '\n',
    );
...
describe('scanRepo', () => {
  ...
  it('passes since option to git log args', async () => {
    ...
    expect(spawnCalls[0].args).toEqual(expect.arrayContaining(['--since=2026-02-01']));
  });
...
describe('parseChurnLog', () => {
  ...
  it('handles binary files (- for insertions/deletions)', () => {
    const output = ['cccc00000000000000000000000000000000cccc', '-\t-\timage.png'].join('\n');
...
describe('GitLogLineParser', () => {
  it('yields a commit when a new header line arrives', () => {
    ...
    // First commit lines
    expect(
      parser.processLine('abc123|alice@acme.com|Alice|2026-02-20T10:00:00Z|feat: add button'),
    ).toBeNull();
...
describe('calculateFastChurnRate', () => {
  ...
  function authorLog(
    commits: Array<{ hash: string; files: Array<[number, number, string]> }>,
  ): string {
    return commits
  ...
  function windowLog(commits: Array<{ hash: string; subject: string; files: string[] }>): string {
    return commits.map((c) => [`${c.hash}|${c.subject}`, ...c.files].join('\n')).join('\n\n');
  }
...
describe('classifyGitError', () => {
  ...
  it('classifies unknown revision as expected', () => {
    const info = classifyGitError(
      new Error('fatal: unknown revision or path not in the working tree'),
    );
    expect(info.severity).toBe('expected');
  });
  ...
  it('classifies ENOENT as transient', () => {
    const info = classifyGitError(
      new Error("ENOENT: no such file or directory, stat '/missing-repo'"),
    );
