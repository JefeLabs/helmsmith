...
describe('SQLite store', () => {
  ...
  it('round-trips a record through SQLite schema', () => {
    ...
    insert.run({
      ...
      intent_feat: record.intent?.feat ?? 0,
      intent_fix: record.intent?.fix ?? 0,
      intent_refactor: record.intent?.refactor ?? 0,
      intent_docs: record.intent?.docs ?? 0,
      intent_test: record.intent?.test ?? 0,
      intent_chore: record.intent?.chore ?? 0,
      intent_other: record.intent?.other ?? 0,
      app_files: record.filetype.app.files,
      app_files_added: record.filetype.app.filesAdded ?? 0,
      app_files_deleted: record.filetype.app.filesDeleted ?? 0,
      app_ins: record.filetype.app.insertions,
      app_del: record.filetype.app.deletions,
      test_files: record.filetype.test.files,
      test_files_added: record.filetype.test.filesAdded ?? 0,
      test_files_deleted: record.filetype.test.filesDeleted ?? 0,
      test_ins: record.filetype.test.insertions,
      test_del: record.filetype.test.deletions,
      config_files: record.filetype.config.files,
      config_files_added: record.filetype.config.filesAdded ?? 0,
      config_files_deleted: record.filetype.config.filesDeleted ?? 0,
      config_ins: record.filetype.config.insertions,
      config_del: record.filetype.config.deletions,
      storybook_files: record.filetype.storybook.files,
      storybook_files_added: record.filetype.storybook.filesAdded ?? 0,
      storybook_files_deleted: record.filetype.storybook.filesDeleted ?? 0,
      storybook_ins: record.filetype.storybook.insertions,
      storybook_del: record.filetype.storybook.deletions,
      doc_files: record.filetype.doc?.files ?? 0,
      doc_files_added: record.filetype.doc?.filesAdded ?? 0,
      doc_files_deleted: record.filetype.doc?.filesDeleted ?? 0,
      doc_ins: record.filetype.doc?.insertions ?? 0,
      doc_del: record.filetype.doc?.deletions ?? 0,
    });

    const row = db.prepare('SELECT * FROM records WHERE member = ?').get('Alice') as Record<
      string,
      unknown
    >;
  ...
  it('merges records with ON CONFLICT DO UPDATE (additive)', () => {
    ...
    const row = db.prepare('SELECT commits, active_days FROM records').get() as {
      commits: number;
      active_days: number;
    };
    ...
    expect((db.prepare('SELECT COUNT(*) as cnt FROM records').get() as { cnt: number }).cnt).toBe(
      1,
    );
  ...
  it('stores and retrieves enrichments', () => {
    ...
    db.prepare('INSERT INTO enrichments VALUES (?, ?, ?, ?, ?, ?)').run(
      'Alice::2026-W10::app',
      3,
      2,
      14.5,
  ...
  it('getMetaTimestamps returns both timestamps when present', () => {
    ...
    db.prepare('INSERT INTO meta (key, value) VALUES (?, ?)').run(
      'commits_last_updated',
      '2026-03-10T12:00:00Z',
    );
    db.prepare('INSERT INTO meta (key, value) VALUES (?, ?)').run(
      'enrichments_last_updated',
  ...
  describe('queryRollup SQL aggregation', () => {
    ...
    it('applies multiple filters simultaneously', () => {
      ...
      const rows = runRollupSQL(db, 'member', 'WHERE org = @org AND team = @team', {
        org: 'Acme',
        team: 'FE',
      });
      expect(rows).toHaveLength(1);
    ...
    it('returns empty result for no matching records', () => {
      ...
      const rows = runRollupSQL(db, 'member', 'WHERE week = @week', { week: '2026-W99' });
      expect(rows).toHaveLength(0);
