...
import { exportData, flattenRecord, recordsToCsv } from '../commands/export-data.js';
...
describe('recordsToCsv', () => {
  ...
  it('preserves one row per record (member × week × repo)', () => {
    const csv = recordsToCsv([makeRecord({ repo: 'web-app' }), makeRecord({ repo: 'api' })]);
    const lines = csv.trimEnd().split('\n');
...
describe('exportData', () => {
  ...
  it('shows error when no records exist', async () => {
    ...
    expect(stderrSpy).toHaveBeenCalledWith('No records to export. Run "gitradar scan" first.');
