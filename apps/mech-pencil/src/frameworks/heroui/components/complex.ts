...
const btn = (id: string, s: string, p: T, primary: boolean): Child =>
  frame(
    id,
    {
      ...
      padding: [16, 10],
      cornerRadius: p.rmd,
      fill: primary ? p.acc : p.surf,
      stroke: primary ? undefined : { thickness: p.bw, fill: p.bd },
    },
    [
      text(`${id}-l`, s, {
        fill: primary ? p.accFg : p.fg,
        fontFamily: p.ff,
        fontSize: p.fm,
        fontWeight: '600',
      }),
    ...
    items.map((it, i) =>
      frame(
        `${id}-${i}`,
        {
          ...
          padding: [10, 8],
          cornerRadius: p.rsm,
          fill: i === selected ? p.soft : 'transparent',
        },
        [
          text(`${id}-${i}-l`, it, {
            fill: i === selected ? p.acc : p.fg,
            fontFamily: p.ff,
            fontSize: p.fm,
  ...
  const grp = (id: string, lbl: string, val: string) =>
    frame(id, { layout: 'vertical', width: 'fill_container', gap: 6 }, [
      ...
      frame(
        ...
        [text(`${id}-in-t`, val, { fill: p.fph, fontFamily: p.ff, fontSize: p.fm })],
      ),
    ]);
  return comp(
    frame('form', { name: 'Form', width: 360, layout: 'vertical', gap: 16 }, [
      ...
      frame(
        'form-actions',
        { layout: 'horizontal', width: 'fill_container', justifyContent: 'end', gap: 12 },
        [btn('form-cancel', 'Cancel', p, false), btn('form-submit', 'Submit', p, true)],
      ),
    ]),
    'organism',
  );
...
export function buildTable(ctx: BuildContext): Child {
  ...
  const cell = (id: string, s: string, head: boolean) =>
    frame(
      ...
      [
        text(`${id}-t`, s, {
          fill: head ? p.mut : p.fg,
          fontFamily: p.ff,
          fontSize: head ? p.fs : p.fm,
  ...
  const row = (id: string, cells: string[], head: boolean) =>
    frame(
      id,
      {
        name: head ? 'Header' : 'Row',
        layout: 'horizontal',
        width: 'fill_container',
        fill: head ? p.soft : p.surf,
        stroke: { thickness: { bottom: p.bw }, fill: p.bd },
      },
      cells.map((cv, i) => cell(`${id}-c${i}`, cv, head)),
    );
  return comp(
    frame(
      'table',
...
export function buildListBox(ctx: BuildContext): Child {
  ...
  return comp(
    frame(
      ...
      ['Profile', 'Billing', 'Team', 'Settings'].map((it, i) =>
        frame(
          `list-box-${i}`,
          {
            name: it,
            ...
            padding: [10, 8],
            cornerRadius: p.rsm,
            fill: i === 1 ? p.soft : 'transparent',
          },
          [
            text(`list-box-${i}-l`, it, {
              fill: i === 1 ? p.acc : p.fg,
              fontFamily: p.ff,
              fontSize: p.fm,
  ...
  return frame(
    ...
    [
      frame(
        ...
        [
          text(`${prefix}-${i}-t`, title, {
            fill: p.fg,
            fontFamily: p.ff,
            fontSize: p.fm,
            fontWeight: '600',
          }),
          text(`${prefix}-${i}-c`, open ? '▾' : '▸', {
            fill: p.mut,
            fontFamily: p.ff,
            fontSize: p.fm,
          }),
      ...
      ...(open
        ? [
            frame(
              ...
              [
                text(`${prefix}-${i}-bt`, 'Details for this section live here.', {
                  fill: p.mut,
                  fontFamily: p.ff,
                  fontSize: p.fs,
...
export function buildDisclosureGroup(ctx: BuildContext): Child {
  ...
  return comp(
    frame(
      'disclosure-group',
      { name: 'DisclosureGroup', width: 360, layout: 'vertical', gap: 12 },
      [0, 1].map((i) =>
        frame(
          `disclosure-group-${i}`,
          {
          ...
          [
            frame(
              ...
              [
                text(`disclosure-group-${i}-t`, i === 0 ? 'Account' : 'Security', {
                  fill: p.fg,
                  fontFamily: p.ff,
                  fontSize: p.fm,
                  fontWeight: '600',
                }),
                text(`disclosure-group-${i}-c`, i === 0 ? '▾' : '▸', {
                  fill: p.mut,
                  fontFamily: p.ff,
                  fontSize: p.fm,
                }),
...
export function buildToolbar(ctx: BuildContext): Child {
  ...
  const ic = (id: string, g: string) =>
    frame(
      ...
      [text(`${id}-g`, g, { fill: p.fg, fontFamily: p.ff, fontSize: p.fm })],
    );
  return comp(
    frame(
      'toolbar',
      ...
      [0, 1, 2, 3, 4, 5, 6].map((cI) => {
        ...
        return frame(
          `${prefix}-r${r}-${cI}`,
          {
            ...
            alignItems: 'center',
            justifyContent: 'center',
            cornerRadius: sel ? FULL : 0,
            fill: sel ? p.acc : inRange ? p.soft : 'transparent',
          },
          [
            text(`${prefix}-r${r}-${cI}-t`, inMonth ? String(day) : '', {
              fill: sel ? p.accFg : inRange ? p.acc : inMonth ? p.fg : p.mut,
              fontFamily: p.ff,
              fontSize: p.fs,
            }),
          ],
...
export function buildCalendar(ctx: BuildContext): Child {
  return comp(
    frame('calendar', { name: 'Calendar', layout: 'vertical', width: 'fit_content' }, [
      calendarGrid('calendar', P(ctx), false),
    ]),
    'organism',
  );
}
export function buildRangeCalendar(ctx: BuildContext): Child {
  return comp(
    frame('range-calendar', { name: 'RangeCalendar', layout: 'vertical', width: 'fit_content' }, [
      calendarGrid('range-calendar', P(ctx), true),
    ]),
    'organism',
  );
}
...
export function buildDatePicker(ctx: BuildContext): Child {
  const p = P(ctx);
  return comp(
    frame('date-picker', { name: 'DatePicker', width: 280, layout: 'vertical', gap: 8 }, [
      fld('date-picker-f', p, '14 / 06 / 2026', false),
      calendarGrid('date-picker', p, false),
    ]),
    'organism',
  );
}
...
export function buildDateRangePicker(ctx: BuildContext): Child {
  ...
  return comp(
    frame(
      'date-range-picker',
      { name: 'DateRangePicker', width: 320, layout: 'vertical', gap: 8 },
      [
        frame('date-range-picker-row', { layout: 'horizontal', width: 'fill_container', gap: 8 }, [
          fld('date-range-picker-s', p, 'Jun 10', false),
          fld('date-range-picker-e', p, 'Jun 16', false),
        ]),
  ...
  return comp(
    frame(
      ...
      [
        ...
        frame(
          'color-picker-row',
          { layout: 'horizontal', width: 'fill_container', alignItems: 'center', gap: 8 },
          [
...
export function buildAutocomplete(ctx: BuildContext): Child {
  const p = P(ctx);
  return comp(
    frame('autocomplete', { name: 'Autocomplete', width: 260, layout: 'vertical', gap: 8 }, [
      fld('autocomplete-f', p, 'Type to search…'),
      menu('autocomplete-m', p, ['Argentina', 'Australia', 'Austria'], 0),
    ]),
    'organism',
  );
}
export function buildComboBox(ctx: BuildContext): Child {
  const p = P(ctx);
  return comp(
    frame('combo-box', { name: 'ComboBox', width: 260, layout: 'vertical', gap: 8 }, [
      fld('combo-box-f', p, 'Select a framework', false),
      menu('combo-box-m', p, ['React', 'Vue', 'Svelte', 'Solid'], 0),
    ]),
    'organism',
  );
}
export function buildSelect(ctx: BuildContext): Child {
  const p = P(ctx);
  return comp(
    frame('select', { name: 'Select', width: 260, layout: 'vertical', gap: 8 }, [
      fld('select-f', p, 'Choose a plan', true),
      menu('select-m', p, ['Starter', 'Team', 'Enterprise'], 1),
    ]),
    'organism',
  );
}
  ...
  return frame(
    ...
    [
      ...
      frame(
        `${id}-ft`,
        { layout: 'horizontal', width: 'fill_container', justifyContent: 'end', gap: 12 },
        [btn(`${id}-cancel`, 'Cancel', p, false), btn(`${id}-ok`, 'Confirm', p, true)],
      ),
    ],
  );
}
...
export function buildModal(ctx: BuildContext): Child {
  return comp(
    dialog(
      ...
      P(ctx),
      'Modal title',
      'Modal content explaining the action the user is about to take.',
      480,
    ),
    'organism',
  );
  ...
  return comp(
    frame(
      'drawer',
      {
        ...
        layout: 'vertical',
        fill: p.surf,
        stroke: { thickness: { left: p.bw }, fill: p.bd },
        effect: SHADOW,
      },
      [
        frame(
        ...
        frame(
          'drawer-body',
          { layout: 'vertical', width: 'fill_container', gap: 12, padding: 20 },
          [fld('drawer-f1', p, 'Category', true), fld('drawer-f2', p, 'Status', true)],
        ),
      ],
    ),
    'organism',
...
export function buildPopover(ctx: BuildContext): Child {
  ...
  return comp(
    frame(
      ...
      [
        text('popover-title', 'Quick tip', {
          fill: p.fg,
          fontFamily: p.ff,
          fontSize: p.fm,
          fontWeight: '700',
        }),
        text('popover-body', 'Popovers float above content and point at their trigger.', {
          fill: p.mut,
          fontFamily: p.ff,
          fontSize: p.fs,
          textGrowth: 'fixed-width',
    ...
    frame(
      ...
      [
        ...
        frame('toast-content', { layout: 'vertical', width: 'fill_container', gap: 4 }, [
          text('toast-title', 'Changes saved', {
            fill: p.fg,
            fontFamily: p.ff,
            fontSize: p.fm,
            fontWeight: '600',
          }),
          text('toast-body', 'Your preferences were updated.', {
            fill: p.mut,
            fontFamily: p.ff,
            fontSize: p.fs,
          }),
