...
function buildGridsPage(ctx: MockupContext, gs: GridSpacingFoundation): Child[] {
  ...
  const spaceRows = gs.spaceKeys.map((key) =>
    frame(nid('srow'), { name: key, layout: 'horizontal', gap: 12, alignItems: 'center' }, [
      rect(nid('sbar'), {
        width: ctx.token(key),
        height: 14,
        fill: ctx.token('color.accent'),
        cornerRadius: 3,
      }),
      text(nid('slabel'), key.replace('space.', 'space-'), {
        fill: muted,
        fontFamily: monoFam,
        fontSize: 11,
      }),
  ...
  const spacing = frame('fg-spacing', { name: 'Spacing scale', layout: 'vertical', gap: 8 }, [
    text('fg-spacing-h', 'Spacing scale', {
      fill: fg,
      fontFamily: fam,
      fontSize: 14,
      fontWeight: '600',
  ...
  const grid = frame('fg-grid', { name: 'Layout grid', layout: 'vertical', gap: 8 }, [
    ...
    frame(
      'fg-grid-demo',
      {
        name: 'columns',
        layout: 'horizontal',
        gap: ctx.token(gs.gutterKey),
        width: GRID_DEMO_WIDTH,
      },
      columns,
    ),
