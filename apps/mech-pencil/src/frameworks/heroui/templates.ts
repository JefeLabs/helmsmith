...
function buildPrimary(ctx: MockupContext, vp: Viewport): Child {
  ...
  const cards = [0, 1, 2, 3].map((i) =>
    ref(id(`card-${i}`), ctx.component('card'), { width: 'fill_container' }),
  );
  const content = frame(
    id('content'),
    {
      ...
      layout: 'vertical',
      width: 'fill_container',
      gap: ctx.token('space.6'),
      padding: ctx.token('space.6'),
    },
    [
      text(id('title'), 'Dashboard', {
        fill: ctx.token('color.foreground'),
        fontFamily: ctx.token('font.display'),
        fontSize: ctx.token('font.h2.size'),
        fontWeight: '700',
      }),
      frame(
        id('grid'),
        {
          name: 'Grid',
          layout: isMobile(vp) ? 'vertical' : 'horizontal',
          width: 'fill_container',
          gap: ctx.token('grid.gutter'),
        },
        cards,
      ),
    ],
        ...
        [
          frame(
            id('sidebar'),
            {
              ...
              width: 240,
              height: 'fill_container',
              fill: ctx.token('color.surface-secondary'),
              padding: ctx.token('space.4'),
              gap: ctx.token('space.2'),
            },
            [
              text(id('nav'), 'Navigation', {
...
function buildSecondary(ctx: MockupContext, vp: Viewport): Child {
  ...
  const sections = [0, 1].map((i) =>
    ref(id(`sec-${i}`), ctx.component('card'), { width: 'fill_container' }),
  );
  const content = frame(
    id('content'),
    {
      ...
      layout: 'vertical',
      width: 'fill_container',
      gap: ctx.token('space.5'),
      padding: ctx.token('space.6'),
    },
    [
      text(id('crumb'), 'Jobs / job_a1b2c3d4', {
        fill: ctx.token('color.muted'),
        fontFamily: ctx.token('font.mono'),
        fontSize: 12,
      }),
      text(id('title'), 'Job detail', {
        fill: ctx.token('color.foreground'),
        fontFamily: ctx.token('font.display'),
        fontSize: ctx.token('font.h2.size'),
        fontWeight: '700',
...
function buildTertiary(ctx: MockupContext, vp: Viewport): Child {
  ...
  const form = frame(
    id('form'),
    {
      name: 'Settings form',
      layout: 'vertical',
      width: isMobile(vp) ? 'fill_container' : 560,
      gap: ctx.token('space.5'),
      padding: ctx.token('space.6'),
    },
    [
      text(id('title'), 'Settings', {
        fill: ctx.token('color.foreground'),
        fontFamily: ctx.token('font.display'),
        fontSize: ctx.token('font.h2.size'),
        fontWeight: '700',
      ...
      frame(
        id('actions'),
        { name: 'Actions', layout: 'horizontal', gap: ctx.token('space.3'), alignItems: 'center' },
        [ref(id('save'), ctx.component('button'))],
      ),
    ],
  );
...
function buildModal(ctx: MockupContext, vp: Viewport): Child {
  ...
  const dialog = frame(
    id('dialog'),
    {
      name: 'Dialog',
      layout: 'vertical',
      width: isMobile(vp) ? 'fill_container' : 420,
      gap: ctx.token('space.4'),
      padding: ctx.token('space.6'),
      fill: ctx.token('color.surface'),
      cornerRadius: ctx.token('radius.lg'),
    },
    [
      text(id('title'), 'Confirm & start', {
        fill: ctx.token('color.foreground'),
        fontFamily: ctx.token('font.display'),
        fontSize: ctx.token('font.h3.size'),
        fontWeight: '600',
      }),
      text(id('body'), 'This will spin up agentx-job and open a PR when accepted.', {
        fill: ctx.token('color.muted'),
      ...
      frame(
        id('actions'),
        {
          name: 'Actions',
          layout: 'horizontal',
          gap: ctx.token('space.3'),
          justifyContent: 'end',
          width: 'fill_container',
        },
        [ref(id('confirm'), ctx.component('button'))],
      ),
    ],
  );
