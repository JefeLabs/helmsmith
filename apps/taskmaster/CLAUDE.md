...
8. **Auth scope (toolbox interop)** — Taskmaster's `src/auth/` predates `@helmsmith/agent-auth`. The OAuth Device Flow login (since `dc1fb5d`) delegates to `@helmsmith/agent-auth`'s `loginGitHubCopilot` for the OAuth steps. The chat-call layer (`call-ai.ts`, `providers/*.ts`) intentionally does NOT delegate to `@helmsmith/agent-adapter` — taskmaster's `auth.json` schema (multi-provider with `active_provider`, plus PKCE state for Anthropic Claude.ai subscription) is broader than agent-auth's `AuthStore` schema. Schema convergence is future work; tracked as either (a) migrate taskmaster's auth.json to AuthStore's schema with a one-time migration script, or (b) extend agent-auth to model multi-provider + custom-fields per provider. Until that decision lands, the chat-call layer stays taskmaster-local.
