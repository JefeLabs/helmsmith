...
**Module package:** `com.jefelabs.helmsmith.controlplane.context`
**Companion documents:**
...
- **Reuses TS chunker engine.** Spring shells out to the published `@helmsmith/context-loader` CLI (`agentx-load`) for actual ingestion. No reimplementation of chunkers in Java.
- **Same Neo4j schema as edge.** Defined by `@helmsmith/context-loader-schema` package; both edge and central run the same Cypher migrations.
...
- The chunker pipeline + graph schema are owned by `@helmsmith/context-loader-core` (TS). Central server *invokes* this code, doesn't reimplement.
- Edge sibling: `@helmsmith/edge-context-server` (TS). Same query shape, different scope.
- Cypher schema migrations: shared via `@helmsmith/context-loader-schema` (see companion PRD).
...
| F4 | Source kinds (v1): `oss-package` (npm, cargo, pypi later), `prose-markdown` (path or URL), `crawled-web`, `oss-docs`. Mirrors `@helmsmith/context-loader-core`'s SourceTypeId. |
...
| F23 | Schema migrations on startup via `@helmsmith/context-loader-schema` Cypher files (idempotent). |
...
|---|---|---|---|
| D1 | Reuse `@helmsmith/context-loader` CLI for ingest | One source of truth for chunkers + graph schema; no Java reimplementation. | 2026-05-06 |
