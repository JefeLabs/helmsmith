...
```bash
npm install @helmsmith/cli-kit commander inquirer
```
...
`@helmsmith/agent-auth`) and platform CLIs (which may use different
...
- **Login flows**: live in `@helmsmith/agent-auth` (or whichever auth
