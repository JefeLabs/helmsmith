...
> Architecture peer to `@helmsmith/edge-context-server` (Neo4j-backed
...
Operators + agents talk to this daemon via `@helmsmith/edge-memory-cli`,
...
```sh
pnpm --filter @helmsmith/edge-memory-server test
```
