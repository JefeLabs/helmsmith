...
| 2.4 | ~~**Decide parallelism** — either implement fan-out/join (LangGraph supports multi-target routing; `joinStrategy` becomes real) or delete `joinStrategy` and validate ≤1 sequence edge~~ | L / S | ✅ Done 2026-08-12 (parallelism slice — decided: IMPLEMENT): every sequence edge fires; explicit `joinStrategy` = barrier ('all'/'any'/nOfM, exactly-once) over forward-edge sources via runtime-private channels; validator rejects exceptional edges into joins; both reports deleted fixture-first. v1 caveats in critical-feedback §3 (join hazards row) |
...
| 2.9 | ~~**JobIntent emission** — submit the enforced-and-recorded `job.flowOutput` intent(s) from `job-definition` flows to the JobStateMachine to launch the actual work flow~~ | M | ✅ Done 2026-08-12 (emission slice): `RunJobDeps.onJobIntents` from `finalizeOrPause`; harness-server spawns children through the shared dispatcher path with two-way lineage (`parentJobId`/`spawnedJobIds`) and `spawnErrors` for unspawnable intents |
| 2.6 | ~~**Suspend wake-up scheduling** (timer via job queue; event via bus subscription)~~ | M | ✅ Done 2026-08-12 (suspend-wakeup slice): timer wakes mirror the SLA pattern (restart re-arm incl. expired-while-down); event wakes via `POST /v1/events` with matcher evaluation against `{type, payload}`; the ingress route is the v1 bus seam (3.1 can reuse it) |
...
| 3.3 | ~~**Controlplane Phase 2 validation** via the generated schema — reject bad flows at write time instead of first-load time~~ | M | ✅ Done 2026-08-13 (controlplane /v1/catalog slice): the Spring service serves the harness `GET`/`PUT /v1/catalog` wire contract (same shapes the designer's catalog client parses, outside the `/api` prefix so the designer just repoints its proxy) and gates PUT bodies against the generated `flow-spec.schema.json` (networknt, `#/definitions/Catalog`, vendored resource drift-guarded against the artifact). Schema-shape gate only — semantic rules stay single-sourced in the TS validator at harness load |
