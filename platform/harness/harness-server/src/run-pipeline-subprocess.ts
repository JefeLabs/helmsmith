...
/** Default tsx binary path in the workspace's pnpm-managed ROOT
 *  node_modules/.bin (tsx is a root devDependency, not a package one).
 *  Anchored to this file: src → harness-server → harness → platform →
 *  repo root. Tests + dev use this. Production deployments override to
 *  the runtime they ship with. */
