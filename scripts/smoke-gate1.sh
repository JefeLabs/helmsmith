...
SUBMIT_OUT="$(cd "$WS_ROOT" && pnpm --silent --filter @helmsmith/harness exec tsx src/index.ts submit "$CHANGE" --product "$PRODUCT" 2>&1)"
echo "$SUBMIT_OUT"
...
if (( DO_CLEANUP )); then
  log "reaping…"
  (cd "$WS_ROOT" && pnpm --silent --filter @helmsmith/harness exec tsx src/index.ts reap --job "$JOB_ID" --force)
fi
