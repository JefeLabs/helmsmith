...
import { type ListOptions, runList } from './commands/list.js';
...
const { program } = createCli({
  name: 'skillzkit',
  version: '0.1.0',
  description: 'Catalog of slash commands, agent skills, and multi-phase workflows for Claude Code',
});
...
program
  ...
  .option('--tag <name>', 'Filter to artifacts carrying this tag (cross-persona discovery)')
  .action((options: ListOptions) => runList(options));

program
  .command('search <query>')
  .description('Find commands, workflows, and skills whose slug, description, or tags match')
...
program
  ...
  .action((slugs: string[], options: { target?: string; force?: boolean; dryRun?: boolean }) =>
...
program
  ...
  .option('--state <path>', 'Path to .pencil-workflow-state.json for active-workflow signal')
  .action((slug: string, options: SuggestOptions) => runSuggest(slug, options));
...
program
  ...
  .action(
    (field: string | undefined, value: string | undefined, options: { showSecrets?: boolean }) =>
...
program
  ...
  .option('--bump <level>', 'Version bump level: major | minor | patch (default: patch)')
