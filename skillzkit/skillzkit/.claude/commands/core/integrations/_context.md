...
- `/core:integrations:setup` — full activation; updates all timestamps
- `/core:integrations:setup --check` — verification only;
  `lastVerified`
- `/core:integrations:manifest --review` — explicit review;
  `lastReviewed`
- Credential rotation reminders surface in
  `/core:integrations:credentials --doctor`
