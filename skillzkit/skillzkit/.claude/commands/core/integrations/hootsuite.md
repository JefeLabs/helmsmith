...
2. Verify REST credentials available:
   ...
   if [ -z "$HOOTSUITE_TOKEN" ]; then
     echo "Hootsuite credentials missing. Run /core:integrations:setup hootsuite"
     exit 1
   fi
   ```
...
direct platform integrations (`/core:integrations:linkedin`,
`/core:integrations:instagram`, etc.). Hootsuite's analytics are
...
/core:integrations:hootsuite "schedule a post for tomorrow at 9am
  to LinkedIn-SkoolScout and Instagram-TourneySeason: 'New
  feature launch — read more at...'"

# View queue
/core:integrations:hootsuite "show me what's scheduled this week"

# List channels
/core:integrations:hootsuite "list connected channels"
```

### As delegation target (LinkedIn integration routing here)

When `/core:integrations:linkedin "post update about X"` runs and
...
The user runs `/core:integrations:linkedin "..."` but Hootsuite is
...
|-------|--------------|----------------|
| 401 Unauthorized | Token expired; OAuth refresh needed | Re-run `/core:integrations:setup hootsuite` |
| 403 Forbidden | Token lacks scope for operation | Hootsuite scopes: `offline`, `messages`, etc. Re-auth with required scopes. |
| 404 Channel not found | channelId no longer valid | Channel may have been disconnected in Hootsuite UI. Run `/core:integrations:setup hootsuite --update` to refresh channel list. |
